...
6. **Action Center Tasks From the CLI: Shipped, Undocumented:**
   - **Was:** until early September 2026 the coding agent had no way to action a Quick Form task; every human-in-the-loop chapter meant a browser switch per task, and an unattended test of a HITL flow was impossible from the CLI.
   - **Now (verified 10.09.2026):** `@uipath/tasks-tool` (npm `1.201.0`, published 07.09.2026) installs on first use as `uip tasks`, with `list`, `get`, `assign`, `reassign`, `unassign`, `users` and `complete`. `uip tasks complete <id> --type QuickFormTask --folder-id <folderId> --action Approve --data '{"reviewernote":"yes"}'` completed a Quick Form created by a paused `uip maestro flow debug` run, and the run resumed through the Approve handle with `reviewOutcome` and `reviewerNote` populated. Chapter 07 documents the command next to the Action Center path.
   - **Improvement:** document the tool. Neither the CLI docs' tools list nor the Action Center docs mention it, and the task's `FolderId` needed for `complete` is only visible through `tasks get`, not in `tasks list`. A `--folder-path` alternative and a `list --status Pending` filter would remove the two lookups an agent has to do per task.
